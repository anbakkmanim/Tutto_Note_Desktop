    ...
    <v-container>
       <div v-if="notelist.length > 0" class="select">
        ...
        <div>
          ...
          <v-btn color="nomal" class="but" v-on:click="rollback()" s v-if="this.$route.path === '/trash'"  >복구</v-btn>
      </div>
    </v-container>
    <v-flex
      xs12 md6 lg4
      ...
      >
        <v-container fluid grid-list-lg>
          <v-layout row>
            ...
            <v-flex xs2>
              <div v-if="routePath !== '/trash'">
                <v-btn nomal fab v-on:click="popup(note._id)" ><v-icon>open_in_new</v-icon></v-btn>
              </div>
            </v-flex>
          </v-layout>
        </v-container>
